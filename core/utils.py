...
from .utils_request import request_post
...
def get_company_data(sii_user: SiiUser) -> dict:
    ...
    # Im using the same variable name that is used in the SII but im don't
    # have idea what tf they mean
    ...
    legal_representatives = request_post(
        sii_user,
        "https://misiir.sii.cl/cgi_misii/CViewCarta.cgi",
        sii_user.cookies,
        {"opc": "112"},
    )["representantes"]
    current_partners = request_post(
        sii_user,
        "https://misiir.sii.cl/cgi_misii/CViewCarta.cgi",
        sii_user.cookies,
        {"opc": "112"},
    ...
    user_data = {
        ...
        "start_of_activities": datos_cntr_now["contribuyente"]["fechaInicioActividades"],
        ...
        "current_partners": [
            {
                ...
                "aware_capital": int(
                        partner["aporteEnterado"]),
                "capital_to_find_out": int(
                    partner["aportePorEnterar"]),
                "capital_percentage": float(
                    partner["participacionCapital"]),
                "percentage_utilities": float(
                    partner["participacionUtilidades"]),
...
def download_buying_book(
    ...
    payload = {
        ...
        "data": {
            "rutEmisor": rut_to_dict(
                sii_user.rut)["rut"],
            "dvEmisor": rut_to_dict(
                    sii_user.rut)["dv"],
